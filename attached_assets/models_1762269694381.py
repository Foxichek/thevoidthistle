...
class User(Base):
    ...
    reputation = relationship("UserReputation", uselist=False, cascade="all, delete-orphan", back_populates="user") # <--- НОВОЕ: Связь с репутацией


class UserReputation(Base): # <--- НОВОЕ: Модель репутации
...
class MineLocation(Base):
    ...
    required_level = Column(Integer, nullable=False, server_default='1')
